...
public interface UserRepository extends JpaRepository<User, Long> {
    ...
    @Query(value = "Select u.id,u.email,u.role,u.is_active From user u where u.email = ?1",nativeQuery = true)
    Optional<Object> findByEmailCustom(String email);

    @Query(value = "Select u.id,u.email,u.role,u.is_active From user u where u.id = ?1",nativeQuery = true)
    ...
    @Query(value = "Select u.id,u.email,u.role,u.is_active,u.avatar,u.full_name,u.created_date " +
            ...
            countQuery = "Select count(u.id) from user u WHERE u.is_active = ?1",
            nativeQuery = true)
    Page<Object> findAllByStatus(Pageable pageable,int status);
