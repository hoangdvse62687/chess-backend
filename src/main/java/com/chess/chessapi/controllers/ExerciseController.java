...
public class ExerciseController {
    ...
    public @ResponseBody JsonResult createExercise(@RequestBody @Valid ExerciseCreateViewModel exerciseCreateViewModel, BindingResult bindingResult){
        String message = "";
    ...
    public @ResponseBody JsonResult getExerciseById(@RequestParam("exerciseId") long exerciseId){
        Exercise exercise = this.exerciseService.getById(exerciseId)
    ...
    @PreAuthorize("hasAnyAuthority("+ AppRole.ROLE_LEARNER_AUTHENTICATIION+","
            + AppRole.ROLE_ADMIN_AUTHENTICATIION+","
            + AppRole.ROLE_INSTRUCTOR_AUTHENTICATIION+")")
    public @ResponseBody JsonResult getExerciseByCourseId(@RequestParam("courseId") long courseId){
    ...
    @PreAuthorize("hasAnyAuthority("+ AppRole.ROLE_INSTRUCTOR_AUTHENTICATIION+")")
    public @ResponseBody JsonResult updateExercise(@RequestBody @Valid ExerciseUpdateViewModel exerciseCreateViewModel, BindingResult bindingResult){
        String message = "";
        boolean isSuccess = true;
        if(bindingResult.hasErrors()){
            FieldError fieldError = (FieldError)bindingResult.getAllErrors().get(0);
    ...
    @PreAuthorize("hasAnyAuthority("+ AppRole.ROLE_INSTRUCTOR_AUTHENTICATIION+")")
    public @ResponseBody JsonResult removeExercise(@RequestBody @Valid ExerciseRemoveViewModel exerciseRemoveViewModel, BindingResult bindingResult){
        String message = "";
        boolean isSuccess = true;
        if(bindingResult.hasErrors()){
            FieldError fieldError = (FieldError)bindingResult.getAllErrors().get(0);
